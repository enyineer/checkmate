  ...
  describe("testConnection", () => {
    it("returns success for reachable endpoint", async () => {
      ...
      try {
        ...
        const result = await webhookProvider.testConnection!(config);
    ...
    it("validates config before testing", async () => {
      const invalidConfig = {
        url: "not-a-url",
      } as WebhookConfig;

      const result = await webhookProvider.testConnection!(invalidConfig);
